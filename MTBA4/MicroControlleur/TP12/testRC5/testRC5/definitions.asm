...
; === DATA MEMORY ADDRESS DECLARATIONS ===
.equ	SOFTRAMEND	= 0x1000 ; real end at 0x10ff, 255 address for user variable should be enough
.equ	RC5l	= 0x10ff ; used by RC5 library to store data in RAM
.equ	RC5h	= 0x10fe ; used by RC5 library to store data in RAM
.equ    stateRC5= 0x10fd ;
.equ	MenuStateGlobal	= 0x10fc ; Store the state of the menu currently selected
.equ	AveragePeriod = 0x10fb ; Store the duration of the averaging period
.equ	Threshold = 0x10fa ; Store the radiation Threshold
.equ 	ClickInLastPeriod = 0x10f9
.equ 	ClickTotalH = 0x10f8
.equ 	ClickTotalL = 0x10f7
.equ	AveragePeriodCouter = 0x10f7 ; Store the current value of the averaging period

.equ	DATABUFFERSTART = 0x10f0 ; Store the current value of the averaging period
.equ	DATABUFFERSTOP = 0x1001 ; Store the current value of the averaging period

; === REGSITER LOCATION DECLARATIONS ===
